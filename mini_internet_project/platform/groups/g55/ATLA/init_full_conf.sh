...
vtysh  -c 'conf t' \
 -c 'interface lo' \
 -c 'ip address 55.157.0.1/24' \
 -c 'exit' \
 -c 'interface host' \
 -c 'ip address 55.107.0.2/24' \
 -c 'exit' \
 -c 'router ospf' \
 -c 'network 55.107.0.2/24 area 0' \
 -c 'exit' \
 -c 'router ospf' \
 -c 'ospf router-id 55.157.0.1' \
 -c 'network 55.157.0.1/24 area 0' \
 -c 'exit'\
 ...
 -c 'ip address 55.0.11.2/24' \
 -c 'ip ospf cost 1' \
 -c 'exit' \
 -c 'router ospf' \
 -c 'network 55.0.11.2/24 area 0' \
 -c 'exit' \
 -c 'interface port_MIAM' \
 -c 'ip address 55.0.13.1/24' \
 -c 'ip ospf cost 1' \
 -c 'exit' \
 -c 'router ospf' \
 -c 'network 55.0.13.1/24 area 0' \
 -c 'exit' \
 -c 'exit' -c 'write' 
