...
vtysh  -c 'conf t' \
 -c 'interface lo' \
 -c 'ip address 55.152.0.1/24' \
 -c 'exit' \
 -c 'router ospf' \
 -c 'network 55.200.0.0/16 area 0' \
 -c 'exit'\
 -c 'router ospf' \
 -c 'ospf router-id 55.152.0.1' \
 -c 'network 55.152.0.1/24 area 0' \
 -c 'exit'\
 ...
 -c 'ip address 55.0.1.1/24' \
 -c 'ip ospf cost 1' \
 -c 'exit' \
 -c 'router ospf' \
 -c 'network 55.0.1.1/24 area 0' \
 -c 'exit' \
 -c 'interface port_LOND' \
 -c 'ip address 55.0.2.1/24' \
 -c 'ip ospf cost 1' \
 -c 'exit' \
 -c 'router ospf' \
 -c 'network 55.0.2.1/24 area 0' \
 -c 'exit' \
 -c 'exit' -c 'write' 
